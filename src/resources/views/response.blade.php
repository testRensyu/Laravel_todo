      ...
      <div class="row">
        <div class="col col-md-offset-3 col-md-6">
          <nav class="panel panel-primary">
            <div class="panel-heading">返信</div>
            <div class="panel-body">
              ...
              <div class="panel-heading">
                <p>{{ $bbs->title }}</p>
                <p class="text-right">name:{{ $bbs->name }}</p>
              </div>
              <div class="panel-body">
                <p style="color:{{ $bbs->text_color }}; white-space: pre-wrap;">{{ $bbs->message }}</p>
                @if(!is_null($bbs->image))
                <img src="{{ Storage::url($bbs->image) }}" alt="">
                @endif
              </div>
              <p class="text-right">created_at : {{$bbs->created_at}}</p>
              <div class="panel-footer">
                <form action="{{ route('edit.bbs',['id'=>$bbs->id]) }}" method="post" class="form-button">
                  @csrf 
                  <button type="submit" class="btn btn-info">編集</button>
                </form>
                <form action="" class="form-button">
                  @csrf
                  <button type="reset" class="btn btn-info">削除</button>
                </form>
              </div>
            </div>    
            <!-- 返信データ表示 ここまで -->
            <!-- 返信フォーム ここから -->
              <form action="{{ route('CreateResponse', ['id' => $id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                  <label for="name">名前</label>
                  <input type="text" class="form-control" name="name" id="name" value="Re: {{ old('name') }}" maxlength="30" required>
                </div>
                <div class="form-group">
                  <label for="title">件名</label>
                  <input type="text" class="form-control" name="title" id="title" value="Re: {{ old('title') }}" maxlength="120" required>
                </div>
                <div class="form-group">
